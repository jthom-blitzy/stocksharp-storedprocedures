...
public class PositionRecalculationService
{
	...
	/// <para>
	/// The position is recomputed by <b>folding <see cref="Recalculate"/> over the ENTIRE persisted trade set
	/// from flat</b> - every <c>dbo.Trades</c> row for the portfolio/security, read in chronological
	/// (<c>executed_date</c>, then <c>trade_id</c>) order - starting from (0, 0, 0). The recompute never trusts
	/// the stored (qty, avg_price, realized_pnl); it derives them afresh from the immutable trade history, so it
	/// is <b>self-healing</b> and <b>inherently idempotent</b>: re-running it for the same trade set reproduces
	/// exactly the same result, and a later-arriving backdated trade is folded into its correct chronological
	/// position on the next recompute rather than being appended after already-committed state (review findings
	/// MI-2 / #9). Full replay is the model the AAP mandates (single entry point, self-healing full replay -
	/// AAP 0.6.4); combined with this being the SOLE writer of <c>dbo.Positions</c> and there being no competing
	/// trigger, no residual or repeated path can double-apply a trade (the historical trigger/standalone
	/// double-count hazard, LEGACY_LAYER.md:L74-L89, stays structurally eliminated). Because the recompute reads
	/// the just-inserted trade through the shared transaction, the caller must INSERT the trade into
	/// <c>dbo.Trades</c> before calling this method within the same transaction.
	/// </para>
	/// <para>
	...
	public async Task<PositionRecalcResult> ApplyTradeAsync(SqlConnection connection, SqlTransaction transaction, long orderId, CancellationToken cancellationToken = default)
	{
		...
		// INSERT of the unique key (CWE-362), and tells us whether to UPDATE or INSERT below. We deliberately do
		// NOT read the stored (qty, avg_price, realized_pnl) to fold from: the recompute always replays the full
		// trade history from flat (Step 3), so the stored values are recomputed afresh, never trusted.
		bool positionExists;

		await using (var lockCommand = new SqlCommand(
			"SELECT 1 FROM dbo.Positions WITH (UPDLOCK, HOLDLOCK) WHERE portfolio_id = @portfolio_id AND security_id = @security_id", connection, transaction))
		{
		...
		// Step 3 - full chronological replay (review findings MI-2 / #9; AAP 0.6.4). Fold the pure math over
		// the ENTIRE persisted trade set for this portfolio/security, starting from flat (0, 0, 0), in the same
		// order the removed trigger's cursor used (executed_date, then trade_id). The stored (qty, avg_price,
		// realized_pnl) are deliberately NOT used as a starting point: deriving them afresh from the immutable
		// trade history makes the recompute self-healing and inherently idempotent (a re-run reproduces the same
		// result), and a later-arriving backdated trade is folded into its correct chronological position rather
		// than applied on top of already-committed state. Recalculate rounds to the persisted DECIMAL(18,4)
		// after each step, so each intermediate matches exactly what SQL Server would store.
		var qty = 0m;
		var avgPrice = 0m;
		var realizedPnl = 0m;
